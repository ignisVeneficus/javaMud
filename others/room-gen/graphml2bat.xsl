...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:g="http://graphml.graphdrawing.org/xmlns" xmlns:y="http://www.yworks.com/xml/graphml" version="1.0">
	...
	<xsl:template match="/*">
		...
		<xsl:for-each select="//g:node">
			...
			<xsl:otherwise>
				<xsl:text>call </xsl:text><xsl:value-of select="$pathExec"/><xsl:text> </xsl:text><xsl:value-of select="$pathSource"/><xsl:text> </xsl:text><xsl:value-of select="$prefix"/><xsl:text>_</xsl:text><xsl:value-of select="./g:data[@key=$nameID]"/><xsl:text> </xsl:text><xsl:value-of select="@id"/><xsl:text> </xsl:text><xsl:value-of select="$path"/><xsl:text> </xsl:text><xsl:value-of select="$withMap"/><xsl:text>
</xsl:text>
			</xsl:otherwise>
			</xsl:choose>
		</xsl:for-each>
