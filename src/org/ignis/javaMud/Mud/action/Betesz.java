...
public class Betesz extends Action {
	...
	public boolean process(String paramList, Living source) {
		...
		if(source instanceof Player) {
			if(((Player)source).isGhost()) {
				source.tell("Egy szellem semmit sem tehet be sehonnét!");
				return true;
			}
		}
		List<String> params = parseCommand(paramList, 0);
		if(params.size()==0) {
			source.tell("Mit akarsz betenni?");
			return true;
		}
		...
		if(boxList.size()==0) {
			source.tell("Hová akarod betenni?");
			return true;
		}
		Holder box = source;
		...
		if(t instanceof Living) {
			source.tell("Élőlénybe nem tehetsz be semmit! Használd az "+Defaults.Color_command+"add" +Colorize.RESET+" parancsot!");
			return true;
		}
		...
		if(mindent) {
			...
			for(Entity e:entityList) {
				if(e instanceof Item) {
					if(betesz((Item)e,source,inRoom,senseList,environment)) {
						e.moveObject(source);
						qty++;
					}
				}
	...
	private boolean betesz(Item target,Living source) {
		return true;
	}
	
	private boolean betesz(Item target,Living source,boolean inRoom,List<Sense> senseList, Map<String, Integer> environment) {
		if(inRoom) {
			int res = checkSense(target, senseList, environment);
			if(res!=Perception.ALL) return false;
		}
		
		return betesz(target,source);
	}

	@Override
	public String getHelp() {
		return "Használat: "+Defaults.Color_command+"betesz" +Colorize.RESET + " " + Defaults.Color_command_arg + "<mit>" + Colorize.RESET + " " + Defaults.Color_command_arg + "<hová>" + Colorize.RESET +"\n" +
				"Beteszed a " + Defaults.Color_command_arg + "<mit>" + Colorize.RESET + " tárgyat a "+ Defaults.Color_command_arg + "<hová>" + Colorize.RESET +" tárolóból.\n"+
				"Ha valamiből több is van, és nem az elsőt szeretnéd betenni, akkor egy sorszámmal utahatsz rá, pl: "+Defaults.Color_command+"betesz tőr 2 láda" + Colorize.RESET +" vagy " +Defaults.Color_command+"betesz tőr láda 2" + Colorize.RESET +".\n"+
				"A " + Defaults.Color_command + "betesz mindent <honnét>" + Colorize.RESET + " paranccsal mindent beteszel.";
	}
